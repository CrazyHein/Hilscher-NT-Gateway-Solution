...
if state == nil then
	...
	req_header = ":011700000005000B000306"
	rsp_header = ":01170A"
	port_number = 1
	ecat_rx_offset = 0
	ecat_tx_offset = 0
...
elseif state == STATE_SEND_RESQUEST_TO_UART then
	...
	elseif port_number == 0 then
		req_header = ":021700000005000B000306"
	    rsp_header = ":02170A"
	    port_number = 1
	    ecat_rx_offset = 16
	    ecat_tx_offset = 16
	...
	local bus_data = bus:BusIOReadDirect(ecat_rx_offset, 6)
	local tx_data = req_header .. byte_array_to_hex_ascii(bus_data, 1) .. byte_array_to_hex_ascii(bus_data, 3) .. byte_array_to_hex_ascii(bus_data, 5)
	...
	if uart:PortExchange(tx_data, MAX_RECV_LEN, true, true) then
	    ...
	    bus:BusIOWriteDirect(ecat_tx_offset + 10, string.char(SERIAL_PORT_SENDING_ERROR, 0x00) ,true)
...
elseif state == STATE_WAIT_UART then
    ...
    if status == port.STA_PATTERN_MATCH and rxerror == nil and rx_data then
        if string.len(rx_data) ~= 31 or lrc_string(rx_data, 2, 27, nil) ~= string.sub(rx_data, 28, 29) then
			bus:BusIOWriteDirect(ecat_tx_offset + 10, string.char(SERIAL_PORT_DATA_FORMAT_ERROR, 0x00) ,true)
			state = STATE_ERROR_CLEAR
			util.SetLed("error", true)
        else
            bus:BusIOWriteDirect(ecat_tx_offset,
    ...
    elseif rxerror ~= nil or status ~= nil then
	    bus:BusIOWriteDirect(ecat_tx_offset + 10, string.char(SERIAL_PORT_RECEIVING_ERROR, 0x00) ,true)
        state = STATE_ERROR_CLEAR
        util.SetLed("error", true)
    end
