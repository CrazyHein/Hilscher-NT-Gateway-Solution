...
function uart_open()
    return PortOpen({interfacetype = port.INTERFACE_TYPE_RS485,
                     ...
                     databits = DATABITS,
                     stopbits = STOPBITS,
                     paritymode = port.PARITY_NONE,           -- acknowledge timout = 1 second, character delay time = 10 msec
...
if state == nil then
	...
	WR_RSQ_PREFIX = string.char(02) .. "01013WWRD05000,05,"
	WR_RSP_FIX = string.char(02) .. "0101OK" .. string.char(0x03, 0x0D)

	RD_RSQ_FIX = string.char(02) .. "01013WRDD05005,06" .. string.char(0x03, 0x0D)
	RD_RSP_PREFIX = string.char(02) .. "0101OK"

	FRAME_SUFFIX = string.char(0x03, 0x0D)
...
elseif state == STATE_RWAIT_UART then
    local status, rsp, rxerror = uart:PortIsExchangeDone()
    if status == port.STA_PATTERN_MATCH and rxerror == nil and rsp then
        if string.len(rsp) ~= 33 or string.sub(rsp, 1, string.len(RD_RSP_PREFIX)) ~=  RD_RSP_PREFIX then
			bus:BusIOWriteDirect(12, string.char(SERIAL_PORT_DATA_FORMAT_ERROR, 0x00) ,true)
			state = STATE_ERROR_CLEAR
			util.SetLed("error", true)
        else
		    bus:BusIOWriteDirect(0,
                                 hex_ascii_to_byte_array(rsp, 8) .. hex_ascii_to_byte_array(rsp, 12) .. hex_ascii_to_byte_array(rsp, 16) .. hex_ascii_to_byte_array(rsp, 20) .. hex_ascii_to_byte_array(rsp, 24) .. hex_ascii_to_byte_array(rsp, 28) .. string.char(0x00, 0x00),
				                 true)
            state = STATE_WSEND_RESQUEST_TO_UART
            util.SetLed("run", true)
            util.SetLed("error", false)
